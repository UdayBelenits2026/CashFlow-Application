...
export const authInterceptor: HttpInterceptorFn = (request, next) => {
  ...
  // Centralized auth headers: bearer token + numeric userId (from the JWT).
  const authRequest = authHeader
    ? request.clone({ setHeaders: { Authorization: authHeader, userId: String(userContext.getUserId()) } })
    : request;
