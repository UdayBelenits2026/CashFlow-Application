...
import { applyMockDataFilters } from '../utility/dashboard-mock-filters';
...
export class DashboardApiService {
  ...
  getDashboard(filters?: Partial<DashboardFilterState>): Observable<DashboardApiResponse> {
    return forkJoin({
      summary: this.http.get<ApiEnvelope<DashboardSummaryDto>>(`${this.baseUrl}/summary`, {
        params: { period: 'MONTH' },
      }),
      trend: this.http.get<ApiEnvelope<CashFlowTrendDto>>(`${this.baseUrl}/cash-flow-trend`, {
        params: { period: '6M' },
      }),
      ...
      recentIncome: this.http.get<ApiEnvelope<RecentIncomeDto[]>>(`${this.baseUrl}/recent-income`, {
        params: { limit: '5' },
      }),
      ...
      cashBalance: this.http.get<ApiEnvelope<CashBalanceDto>>(`${this.baseUrl}/cash-balance`, {
      ...
      map((r) => {
        const data: DashboardApiResponse = {
          ...
          spendingByCategoryChart: mapSpendingByCategory(r.spending.data),
          upcomingBills: mapUpcomingBills(r.upcomingBills.data),
          recentTransactions: mapRecentTransactions(r.recentTransactions.data),
          recentIncome: mapRecentIncome(r.recentIncome.data),
          recentExpenses: mapRecentExpenses(r.recentExpenses.data),
          cashBalance: mapCashBalance(r.cashBalance.data),
          widgetConfig: mapConfiguration(r.config.data),
        };
        return applyMockDataFilters(data, filters);
      }),
    );
  }
  ...
  updateWidgetConfig(widgetConfig: DashboardWidgetConfig[]): Observable<DashboardWidgetConfig[]> {
    return this.http
      .put<
        ApiEnvelope<DashboardConfigurationDto>
      >(`${this.baseUrl}/configuration`, toConfigurationDto(widgetConfig))
  ...
  addUpcomingBill(item: DashboardItem): Observable<DashboardItem> {
    return this.http
      .post<
        ApiEnvelope<UpcomingBillDto>
      >(`${this.baseUrl}/upcoming-bills`, toUpcomingBillWriteDto(item))
      .pipe(map((r) => mapUpcomingBill(r.data)));
  }
  ...
  updateUpcomingBill(item: DashboardItem): Observable<DashboardItem> {
    return this.http
      .put<
        ApiEnvelope<UpcomingBillDto>
      >(`${this.baseUrl}/upcoming-bills/${item.id}`, toUpcomingBillWriteDto(item))
      .pipe(map((r) => mapUpcomingBill(r.data)));
  }
  ...
  deleteUpcomingBill(id: number | string): Observable<number | string> {
    return this.http
      .delete<ApiEnvelope<unknown>>(`${this.baseUrl}/upcoming-bills/${id}`)
      .pipe(map(() => id));
  }
  ...
  connectAccount(payload: AccountLinkPayload): Observable<AccountLinkPayload> {
    return this.http.post<AccountLinkPayload>(`${environment.apiBaseUrl}/accounts`, payload);
  }

  // Updates user profile setup preferences via backend API
  saveProfile(payload: ProfileSetupForm): Observable<ProfileSetupForm> {
    return this.http.post<ProfileSetupForm>(`${environment.apiBaseUrl}/profile`, payload);
  }
}
